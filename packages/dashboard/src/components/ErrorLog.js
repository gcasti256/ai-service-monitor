...
export function ErrorLog({ errors, total }) {
    ...
    return (<div className="bg-bg-surface border border-border rounded-lg p-5">
      ...
      <div className="space-y-2">
        {errors.length === 0 ? (<div className="text-text-muted text-sm py-8 text-center">No errors recorded</div>) : (errors.map((err) => (<div key={err.id} className="border border-border/50 rounded-lg overflow-hidden">
              <button onClick={() => setExpanded(expanded === err.id ? null : err.id)} className="w-full flex items-center justify-between p-3 hover:bg-bg-hover transition-colors text-left">
              ...
              {expanded === err.id && (<div className="border-t border-border/50 p-3 bg-bg-elevated text-sm space-y-2">
                  <div>
