    ...
    return (<div className="bg-bg-surface border border-border rounded-lg p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <select value={filters.status || ''} onChange={e => setFilters(f => ({ ...f, status: e.target.value || '' }))} className="px-2 py-1 bg-bg-elevated border border-border rounded text-text-secondary text-xs">
            <option value="">All Status</option>
            <option value="success">Success</option>
            <option value="error">Error</option>
          </select>
          <select value={filters.provider || ''} onChange={e => setFilters(f => ({ ...f, provider: e.target.value || '' }))} className="px-2 py-1 bg-bg-elevated border border-border rounded text-text-secondary text-xs">
      ...
      {loading && !data ? (<div className="text-text-muted text-sm py-8 text-center">Loading traces...</div>) : (<>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(data?.traces || []).map((trace) => (<tr key={trace.id} onClick={() => setSelectedTraceId(trace.trace_id)} className="border-b border-border/30 hover:bg-bg-hover cursor-pointer transition-colors">
          ...
          {data && (<div className="mt-3 text-text-muted text-xs text-right">
              Showing {data.traces.length} of {data.total} traces
            </div>)}
        </>)}

      {selectedTraceId && (<TraceDetail traceId={selectedTraceId} onClose={() => setSelectedTraceId(null)}/>)}
    </div>);
}
